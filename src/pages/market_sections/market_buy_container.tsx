...
import { SERVICE_FEE_CUT, STANDARD_DECIMALS } from '../../common/constants'
...
//import { Outcome, Outcomes } from '../../components/market/market_create/steps/outcomes/index'
...
const MarketBuyContainer: React.FC<Props> = (props: Props) => {
  ...
  const feePaid = mulBN(debouncedAmount, bigNumberToNumber(fee, STANDARD_DECIMALS) + SERVICE_FEE_CUT / 100)
  const feeFormatted = `${bigNumberToString(feePaid, collateral.decimals)}
  ${collateral.symbol}`
  const baseCost = debouncedAmount?.sub(feePaid)
  const baseCostFormatted = `${bigNumberToString(baseCost || Zero, collateral.decimals)}
  ${collateral.symbol}`

  const potentialProfit = tradedShares.isZero()
    ? new BigNumber(0)
    : tradedShares
        .sub(amount || Zero)
        .div(baseCost)
        .mul(100)
  const potentialProfitFormatted = `${potentialProfit.toString()}.00%`
  //const potentialProfitFormatted = `${bigNumberToString(potentialProfit, collateral.decimals)} ${collateral.symbol}`
  //console.log('feePaid', collateral.decimals, potentialProfit.toString(), potentialProfitFormatted)
