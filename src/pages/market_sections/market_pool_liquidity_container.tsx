...
import {
  ...
  calcTotalCollateralShortage,
  calcTotalDepositShareSurplus,
  calcTotalWithdrawShareSurplus,
...
const MarketPoolLiquidityContainer: React.FC<Props> = (props: Props) => {
  ...
  const { address: marketMakerAddress, balances, fee, question, totalPoolShares, userEarnings } = marketMakerData
  ...
  //Demiurge
  let resolutionDate = new Date('Fri Dec 30 2999 06:00:00 GMT-0500 (Eastern Standard Time)').getTime()
  if (NEW_RESOLUTION_DATE.filter(item => item.question_title === question.title).map(item => item.new_date)[0]) {
    resolutionDate = Date.parse(
      NEW_RESOLUTION_DATE.filter(item => item.question_title === question.title).map(item => item.new_date)[0],
    )
  } else {
    resolutionDate = marketMakerData.question.resolution.getTime()
  }
  //console.log('resolution date', new Date(resolutionDate))
  ...
  //Demiurge
  const totalUnrealizedPotentialFailurePnL = Number(0)
  const totalUnrealizedPotentialSuccessPnL = Number(0)
  const totalUnrealizedOutcomeToken = Zero
  const totalUnrealizedCollateral = Zero
  const totalPriceData = null

  const totalCollateralShortage = calcTotalCollateralShortage(
    amountToFund || Zero,
    balances.map(b => b.holdings),
    totalPoolShares,
    amountToRemove || Zero,
  ...
  const totalDepositShareSurplus = calcTotalDepositShareSurplus(
    totalPoolShares,
    balances.map(b => b.holdings),
    balances.map(b => b.shares),
    amountToFund || Zero,
    amountToRemove || Zero,
  )

  const totalWithdrawShareSurplus = calcTotalWithdrawShareSurplus(
    totalPoolShares,
    balances.map(b => b.holdings),
    balances.map(b => b.shares),
    amountToFund || Zero,
  ...
  const totalShareSurplus = totalDepositShareSurplus.add(totalWithdrawShareSurplus)
