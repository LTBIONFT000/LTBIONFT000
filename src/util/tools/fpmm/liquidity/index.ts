...
import { SERVICE_FEE_CUT } from '../../../../common/constants'
...
export const calcSharesBought = (
  ...
  const sendAmountsAfterAddingFunding = calcAddFundingSendAmounts(
    collateralTokenAmount.mul(100 - SERVICE_FEE_CUT).div(100),
    balances,
    poolShares,
  )
...
/**
 * Compute the deviation of collateral from the initial deposit can be obtained via merging after the user
 * removed `removedFunds` of pool shares or added 'addedFunds' of pool shares
 * @param addedFunds - the amount of collateral being added to the market maker as liquidity
 * @param poolShareSupply - the total supply of liquidity pool tokens
 * @param removedFunds - the amount of liquidity pool tokens being sent to the market maker in return for underlying outcome tokens
 * @param poolBalances - the market maker's balances of outcome tokens
 * @param depositedTokens - the market maker's new outcome tokens after removing removedFund
 * @param fundingBalance - the market maker's balances of funding
 * @param initialOdds - an array of numbers proportional to the initial estimate of the probability of each outcome
 */
export const calcTotalCollateralShortage = (
  addedFunds: BigNumber,
  poolBalances: BigNumber[],
  poolShareSupply: BigNumber,
  removedFunds: BigNumber,
  ...
  //const initialFundingBalance = calcInitialFundingSendAmounts(addedFunds, distributionHint)
  //const depositedTokens = calcDepositedTokens(removedFunds, poolBalances, poolShareSupply)
  const poolTokens = calcPoolTokens(addedFunds, poolBalances, poolShareSupply)
  const tokenDisparity = poolShareSupply.sub(fundingBalance)
  return tokenDisparity
    .add(addedFunds)
    .sub(poolTokens)
    .add(removedFunds)
    .sub(depositedTokens)
...
/**
 * Compute the number of outcome tokens that will be sent to the user by the Market Maker
 * after adding 'addedFunds' of pool shares
 * @param addedFunds - the amount of collateral being added to the market maker as liquidity
 * @param poolShareSupply - the total supply of liquidity pool tokens
 * @param removedFunds - the amount of liquidity pool tokens being sent to the market maker in return for underlying outcome tokens
 * @param poolBalances - the market maker's balances of outcome tokens
 * @param depositedTokens - the market maker's new outcome tokens after removing removedFund
 */
export const calcTotalDepositShareSurplus = (
  poolShareSupply: BigNumber,
  poolBalances: BigNumber[],
  shares: BigNumber[],
  addedFunds: BigNumber,
  ...
  const sendAmountsAfterAddingFunding = calcAddFundingSendAmounts(addedFunds, poolBalances, poolShareSupply)
  const sharesAfterAddingFunding = sendAmountsAfterAddingFunding
    ? shares.map((balance, i) => balance.add(sendAmountsAfterAddingFunding[i]))
    : shares.map(share => share)

  const sendAmountsAfterRemovingFunding = calcRemoveFundingSendAmounts(removedFunds, poolBalances, poolShareSupply)
  const depositedTokens = sendAmountsAfterRemovingFunding.reduce((min: BigNumber, amount: BigNumber) =>
    amount.lt(min) ? amount : min,
  )
  const sharesAfterRemovingFunding = shares.map((share, i) => {
  ...
  return sharesAfterAddingFunding[0].sub(sharesAfterRemovingFunding[0])
  //return sharesAfterAddingFunding.map((balance, i) => balance.sub(sharesAfterRemovingFunding[i]))
  //
}
 ...
 * @param depositedTokens - the market maker's new outcome tokens after removing removedFund
 */
export const calcTotalWithdrawShareSurplus = (
  poolShareSupply: BigNumber,
  poolBalances: BigNumber[],
  shares: BigNumber[],
  addedFunds: BigNumber,
  ...
  return sharesAfterRemovingFunding[1].sub(sharesAfterAddingFunding[1])
  //return sharesAfterAddingFunding.map((balance, i) => balance.sub(sharesAfterRemovingFunding[i]))
  //
}
