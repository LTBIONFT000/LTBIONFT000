...
import {
  ...
  DEFAULT_LIQUIDITY_PROTECTION_SUPPLYTO_ADDRESS_RINKEBY,
  DEFAULT_SERVICE_FEE_SUPPLYTO_ADDRESS_RINKEBY,
...
import { LiquidityProtectionService } from '../dt_lpsd'
...
//Demiurge
export const depositWithServiceFee = async (params: DepositParams) => {
  const { account, amount, collateral, native, service, transactions } = params
  const DTAmount = amount.mul(SERVICE_FEE_CUT).div(100)
  const LPAmount = amount.sub(DTAmount)
  console.log('deposit big DTAmount', DTAmount.toString())
  console.log('deposit big LPAmount', LPAmount.toString())
  if (!service.isSafeApp && collateral.address.toLowerCase() !== native.address.toLowerCase()) {
    // Step 2: Transfer the amount of collateral being spent from the user to the CPK
    transactions.push({
      to: collateral.address,
      data: ERC20Service.encodeTransferFrom(account, DEFAULT_SERVICE_FEE_SUPPLYTO_ADDRESS_RINKEBY, DTAmount),
    })
    transactions.push({
      to: collateral.address,
      data: ERC20Service.encodeTransferFrom(account, service.cpk.address, LPAmount),
    })
  }
  return params
}
...
export const withdraw = async (params: WithdrawParams) => {
  ...
  if (!service.isSafeApp && !amount.isZero()) {
    if (collateral.address.toLowerCase() === wrapper.address.toLowerCase()) {
      ...
      console.log('transaction withdraw', transactions)
    }
  }
  return params
}
...
//Demiurge
export const withdrawWithServiceFee = async (params: WithdrawParams) => {
  const { account, amount, collateral, service, transactions, wrapper } = params
  const DTAmount = amount.mul(SERVICE_FEE_CUT).div(100)
  const LPAmount = amount.sub(DTAmount)
  console.log('withdraw big DTAmount', DTAmount.toString())
  console.log('withdraw big LPAmount', LPAmount.toString())
  if (!service.isSafeApp && !amount.isZero()) {
    if (collateral.address.toLowerCase() === wrapper.address.toLowerCase()) {
      transactions.push({
        to: DEFAULT_SERVICE_FEE_SUPPLYTO_ADDRESS_RINKEBY,
        value: DTAmount.toString(),
      })
      transactions.push({
        to: account,
      ...
      transactions.push({
        to: collateral.address,
        data: ERC20Service.encodeTransfer(DEFAULT_SERVICE_FEE_SUPPLYTO_ADDRESS_RINKEBY, DTAmount),
      })
      transactions.push({
        to: collateral.address,
        data: ERC20Service.encodeTransfer(account, LPAmount),
      })
      console.log('transaction withdraw', transactions)
    }
  }
...
export const buy = async (params: BuyParams) => {
  ...
  console.log('buy amount', amount.toString())
  transactions.push({
...
//Demiurge
export const buyWithServiceFee = async (params: BuyParams) => {
  const { amount, marketMaker, outcomeIndex, transactions } = params
  const DTAmount = amount.mul(SERVICE_FEE_CUT).div(100)
  const buyAmount = amount.sub(DTAmount)
  console.log('buy big DTAmount', DTAmount.toString())
  console.log('buy big buyAmount', buyAmount.toString())
...
//Demiurge
export const addFundsWithServiceFee = async (params: AddFundsParams) => {
  const { account, amount, collateral, marketMaker, transactions } = params
  const DTAmount = amount.mul(SERVICE_FEE_CUT).div(100)
  const LPAmount = amount.sub(DTAmount)
  console.log('add big DTAmount', DTAmount.toString())
  console.log('add big LPAmount', LPAmount.toString())
  transactions.push({
    to: marketMaker.address,
    data: MarketMakerService.encodeAddFunding(LPAmount),
  })
...
interface DemandCompensationParams {
  account: string
  amountToMerge: BigNumber
  collateral: Token
  conditionalTokens: ConditionalTokenService
...
export const payoutCompensation = async (params: DemandCompensationParams) => {
  const { account, amountToMerge, collateral, marketMaker, native, service, transactions } = params
  console.log('cpk address', service.cpk.address)
  console.log('marketmaker address', marketMaker.address)
  console.log('account address', account)
  if (!service.isSafeApp && collateral.address.toLowerCase() !== native.address.toLowerCase()) {
    // Step 2: Transfer the amount of collateral being spent from the user to the CPK
    transactions.push({
      //dt_lpsd address
      to: DEFAULT_LIQUIDITY_PROTECTION_DEMANDFROM_ADDRESS_RINKEBY,
      data: LiquidityProtectionService.encodePayout(account, amountToMerge),
    })
  }
  return params
}

interface SupplyCompensationParams {
  account: string
  amountToMerge: BigNumber
  collateral: Token
  marketMaker: MarketMakerService
...
//Demiurge
export const provideCompensation = async (params: SupplyCompensationParams) => {
  const { account, amountToMerge, collateral, service, transactions, wrapper } = params
  if (!service.isSafeApp && !amountToMerge.isZero()) {
    if (collateral.address.toLowerCase() === wrapper.address.toLowerCase()) {
      transactions.push({
        to: account,
        value: amountToMerge.toString(),
      })
    } else {
      transactions.push({
        to: collateral.address,
        data: ERC20Service.encodeTransferFrom(
          account,
          DEFAULT_LIQUIDITY_PROTECTION_SUPPLYTO_ADDRESS_RINKEBY,
          amountToMerge,
        ),
