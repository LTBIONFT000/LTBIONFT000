...
class CPKService {
  ...
  waitForTransaction = async (txObject: TransactionResult): Promise<TransactionReceipt> => {
    ...
    console.log('TransactionReceipt', txObject)
  ...
  buyOutcomes = async (params: CPKBuyOutcomesParams): Promise<TransactionReceipt> => {
    try {
      const { transaction } = await this.pipe(fee, wrap, approve, depositWithServiceFee, buyWithServiceFee)(params)
      return transaction
    } catch (err) {
  ...
  addFunding = async (params: CPKAddFundingParams): Promise<TransactionReceipt> => {
    try {
      const { transaction } = await this.pipe(fee, wrap, approve, depositWithServiceFee, addFundsWithServiceFee)(params)
      return transaction
    } catch (err) {
  ...
  //Demiurge
  demandProtection = async (params: CPKProtectParams): Promise<TransactionReceipt> => {
    try {
      const { transaction } = await this.pipe(payoutCompensation)(params)
      return transaction
    } catch (err) {
      logger.error(`Error trying to demand liquidity`, err.message)
      throw err
    }
  }

  //Demiurge
  supplyProtection = async (params: CPKProtectParams): Promise<TransactionReceipt> => {
    try {
      const { transaction } = await this.pipe(provideCompensation)(params)
      return transaction
    } catch (err) {
      logger.error(`Error trying to supply liquidity`, err.message)
      throw err
    }
  }
