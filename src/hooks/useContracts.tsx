...
export const useContracts = (context: ConnectedWeb3Context) => {
  ...
  //Demiurge
  const lpsdAddress = getContractAddress(networkId, 'lpsd')
  const lpsd = useMemo(() => new LiquidityProtectionService(lpsdAddress, provider), [provider, lpsdAddress])
  ...
  return useMemo(
    ...
    [conditionalTokens, marketMakerFactory, realitio, oracle, kleros, buildMarketMaker, buildOracle, dxTCR, lpsd],
  )
}
