...
import { DOCUMENT_FAQ, SERVICE_FEE_CUT, STANDARD_DECIMALS } from '../../../common/constants'
...
const MarketSellWrapper: React.FC<Props> = (props: Props) => {
  ...
  const { balances, fee } = marketMakerData
  ...
  return (
    <>
      ...
      <GridTransactionDetails>
        ...
        <div>
          <TransactionDetailsCard>
            ...
            <TransactionDetailsRow
              title={'Total Fee'}
              tooltip={`A ${bigNumberToNumber(fee, STANDARD_DECIMALS) *
                100}% trading fee goes to liquidity providers.\nA ${SERVICE_FEE_CUT}% service fee goes to the platform.`}
              value={`${costFee ? bigNumberToString(costFee, collateral.decimals) : '0.00'} ${collateral.symbol}`}
            />
      ...
      <WarningMessageStyled
        ...
        description=" This market predicts whether a clinical trial will meet its predefined primary endpoints that are duly disclosed to the public in that trial's registration on https://www.clinicaltrials.gov/.  
          This market will resolve to 'Success' if at least one primary endpoint is met accordingly to topline analyses. 
          This market will resolve to 'Failure' if none of the primary endpoint(s) are met or deemed likely to be met according to interim analyses, or terminated for any reasons (low accrual, voluntary stop, etc.). 
          This market will be resolved in good faith according to the official press releases, financial reports or research reports published by the sponsor(s) of the trial (More Info). "
        href={DOCUMENT_FAQ}
      ...
      <StyledButtonContainer borderTop={true} marginTop={isNegativeAmountShares}>
        ...
        <Button buttonType={ButtonType.sellSecondaryLine} disabled={isSellButtonDisabled} onClick={() => finish()}>
          Sell
        </Button>
