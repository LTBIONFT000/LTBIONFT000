...
import { HistoricData, MarketMakerData, Period, Token } from '../../../../../util/types'
...
import { UserPoolData } from '../../../market_pooling/user_pool_data'
...
const UserPoolDataWrapper = styled(UserPoolData)`
  display: none;
`
...
`
export let totalData: { date: string }[] | null = null
...
export const History_select: React.FC<Props> = ({
  ...
  //Demiurge:
  const nToken = { address: '', decimals: 0, symbol: '' }
  //Every blocksPerPeriod/unit number of blocks/holdingsSeries/data need to be aggregated into OHLM data per Datapoint
  //CAUTION: Success for upper choice and Failure for lower choice;
  const rawSuccessData =
    data && data.map((el, i, arr) => Object.entries(arr[i]).filter(a => a.includes('Success') || a.includes('date')))
  const rawFailureData =
    data && data.map((el, i, arr) => Object.entries(arr[i]).filter(a => a.includes('Failure') || a.includes('date')))
  //console.log('unit, data, rawSuccessData, rawFailureData', unit, data, rawSuccessData, rawFailureData)
  ...
  //console.log('SuccessPriceDateData', SuccessPriceDateData)
  //console.log('FailurePriceDateData', FailurePriceDateData)

  function convertToOHLC(
    x: { [k: string]: string }[] | null,
    unit: number,
    ...
    if (x) {
      ...
      //var allDates = [...new Set(x.map(a => a.date))];
      for (let i = 0; i < x.length; i += unit) {
        const tempObject: any = {}
        tempObject.date = x.slice(i + unit - 1, i + unit).map((el, i, arr) => Object.values(el)[1])[0]
        tempObject.open = x
          .slice(i, i + 1)
          .slice(0)
          .map((el, i, arr) => Object.values(el)[0])[0]
        tempObject.high = x
          .slice(i, i + unit)
          .map((el, i, arr) => Object.values(el)[0])
          .map(a => Number(a))
          .reduce((a, b) => {
            return Math.max(a, b)
          })
        ...
        tempObject.close = x
          .slice(i + unit - 1, i + unit)
          .slice(0)
          .map((el, i, arr) => Object.values(el)[0])[0]
        ohlcData.push(tempObject)
      }
    }
    return ohlcData
  }
  const ohlcSuccessData = convertToOHLC(SuccessPriceDateData, unit)
  const ohlcFailureData = convertToOHLC(FailurePriceDateData, unit)
      ...
      {toggleSelect ? (
        ...
        <div>
          <HistoryChart
            ...
            //Demiurge
            ohlcFailureData={ohlcFailureData}
            ohlcSuccessData={ohlcSuccessData}
          ...
          <UserPoolData
            ...
            symbol={''}
            //Demiurge
            totalCollateralShortage={Zero}
            totalEarnings={Zero}
            totalPoolShares={Zero}
            totalPriceData={data}
            totalShareSurplus={Zero}
            totalUnrealizedCollateral={Zero}
            totalUnrealizedOutcomeToken={Zero}
            totalUnrealizedPotentialFailurePnL={NaN}
            totalUnrealizedPotentialSuccessPnL={0}
            totalUserLiquidity={Zero}
            userEarnings={Zero}
          />
        </div>
      )}
