...
const TRExtended = styled(TR as any)<{ clickable?: boolean }>`
  ...
  &:hover td {
    background-color: ${props => (props.clickable ? props.theme.colors.activeListItemBackground : '#010E2C')};
  }
`
...
const TDFlexDiv = styled.div<{ textAlign?: string }>`
  ...
  justify-content: ${props =>
    props.textAlign && 'right' ? 'center' : props.textAlign && 'center' ? 'center' : 'flex-start'};
`
...
export const OutcomeTable = (props: Props) => {
  ...
  const TableCellsAlign = ['center', 'center', 'center', 'center', 'center', 'center', 'center']
  ...
  const renderTableRow = (balanceItem: BalanceItem, outcomeIndex: number) => {
    ...
    //const newPrice = (probabilities[outcomeIndex] / probabilities[outcomeIndex]).toFixed(2)
    ...
    return (
      ...
      >
        ...
        {disabledColumns.includes(OutcomeTableValue.Probability) ? null : (
          <TDStyled textAlign={TableCellsAlign[2]}>
            <TDFlexDiv textAlign={TableCellsAlign[2]}>1 : {(100 / probability).toFixed(2)}</TDFlexDiv>
          </TDStyled>
        )}
