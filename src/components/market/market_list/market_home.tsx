...
`
const FilterControlDropdown = styled(Dropdown)`
  width: 33%;
  height: 40%;
  border: 2px solid ${props => props.theme.colors.tertiary};
  &:hover {
...
export const MarketHome: React.FC<Props> = (props: Props) => {
  ...
  useEffect(() => {
    if (RemoteData.hasData(categories)) {
      ...
      //Demiurge
      console.log('category map1', category, index, item)
      categories.data.filter((ele, ind, arr) => CATEGORIES.includes(arr[ind].id.toLowerCase()))
    }
  }, [category, categories])
  ...
  const categoryItems: Array<DropdownItemProps> =
    RemoteData.hasData(categories) && categories.data.length > 0
      ? ([
          ...
          //DEMIURGE: filter out irrelevant categories
          //ATTN: .filter will stop changing the showing categories.
          ...categories.data
            .filter(item => CATEGORIES.includes(item.id.toLowerCase()))
  ...
  //Demiurge
  //markets.data.slice(0, count).filter(item => item.title.includes('NCT'))
      ...
      <ListCard>
        ...
        <ListWrapper>
          {!isFiltering &&
            ...
            markets.data
              .slice(0, count)
              .filter(item => item.title.includes('NCT'))
              .map(item => {
