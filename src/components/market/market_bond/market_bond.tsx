...
const MarketBondWrapper: React.FC<Props> = (props: Props) => {
  ...
  const bondOutcome = async (isInvalid?: boolean) => {
    ...
    try {
      ...
      //Demiurge
      const rawQ = marketMakerData.question
      let question = marketMakerData.question
      let resolutionDate = new Date('Fri Dec 30 2999 06:00:00 GMT-0500 (Eastern Standard Time)')

      if (NEW_RESOLUTION_DATE.filter(item => item.question_title === rawQ.title).map(item => item.new_date)[0]) {
        resolutionDate = new Date(
          NEW_RESOLUTION_DATE.filter(item => item.question_title === rawQ.title).map(item => item.new_date)[0],
        )
        question = { ...rawQ, resolution: resolutionDate }
        console.log('compare yes', question)
      } else {
        resolutionDate = rawQ.resolution
        console.log('compare no', question.resolution)
      }

      logger.log(`Submit Answer questionId: ${question.resolution}, answer: ${answer}`, bondNativeAssetAmount)
